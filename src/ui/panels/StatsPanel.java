...
public class StatsPanel extends JPanel {
    ...
    public StatsPanel(StudyService service) {
        ...
        var refreshBtn = UIFactory.createButton("Обновить", e -> {
            LOGGER.info("Ручное обновление: Перечитываем файлы с диска...");
            service.reloadSession(); // <--- ГЛАВНОЕ ИСПРАВЛЕНИЕ
            refreshData();
        ...
        var importBtn = UIFactory.createButton("Импорт из txt", e -> {
            ...
            // Используем наш кастомный диалог
            Theme.showInfoDialog(this, result);

            LOGGER.info("Результат импорта: " + result.replace("\n", " | "));
            refreshData();
        });
